...
let { isUrl, sleep, getBuffer, format, parseMention, getRandom, fancy, randomfancy, botpic, tlang } = require("./DATABASE/lib");
const { smsg } = require('./DATABASE/lib/myfuncn')
const { formatp, formatDate, getTime, clockString, runtime, fetchJson, jsonformat, GIFBufferToVideoBuffer, getSizeMedia, generateMessageTag, fancytext } = require('./DATABASE/lib')
...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        const ChangePic = __dirname + "/src/logo.jpg"
...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
        ...
        KingBot.ev.on('messages.upsert', async chatUpdate => {
            ...
            if(chat.key && chat.key.remoteJid === 'status@broadcast'  && Config.auto_read_status==='true'){
            await KingBot.readMessages([chat.key])    
	    }
            if (chat.key && chat.key.remoteJid === 'status@broadcast') return
            try {
                let citel = await smsg(KingBot, JSON.parse(JSON.stringify(chat)), store)
                if (!citel.message) return
                if (citel.chat.endsWith("broadcast")) return;
                if (Config.alwaysonline==='true') {
                    KingBot.sendPresenceUpdate('available', citel.chat)
                }
                var { body } = citel
		 ...
		 if (Config.readmessage==="true" && icmd) {
                ...
                const botNumber = await KingBot.decodeJid(KingBot.user.id)
		...
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                if (icmd) {
                    ...
                    if (cmd) {
                        ...
                        try {
                            cmd.function(KingBot, citel, text,{ args, isCreator, body, budy});
                        } catch (e) {
                ...
                events.commands.map(async(command) => {
                    if (body && command.on === "body") {
                        command.function(KingBot, citel,{args, isCreator, icmd, body, budy});
                        ...
                        command.function(KingBot, citel, args,{isCreator, body, budy});
                    } else if (
                        command.on === "sticker" &&
                ...
                const groupMetadata = citel.isGroup ? await KingBot.groupMetadata(citel.chat)
                    .catch((e) => {}) : "";
...
`,
                                        ...
                                        KingBot.sendMessage(citel.chat, buttonMessage)
                                        return
                                    }

                                    //  console.log('Whatsapp link')
                                    let response = await KingBot.groupInviteCode(citel.chat)
                                    ...
                                    await KingBot.sendMessage(citel.chat, { delete: key })
                                    citel.reply("Group Link Detected!!");

                                    try {
                                        await KingBot.groupParticipantsUpdate(citel.chat, [citel.sender], 'remove')
                                    } catch {
                ...
                const { chatbot } = require('./DATABASE/lib/')
...
async function startcron(time,chat,type){
    ...
    cron.schedule(`${min} ${hr} * * *`, () => {
                (async() => {
                   return await KingBot.groupSettingUpdate(chat, j);
                })()
            }, {
                scheduled: true,
...
async function fooz(){	
 ...
 KingBot.ev.on('group-participants.update', async(anu) => {
            try {
                let metadata = await KingBot.groupMetadata(anu.id)
                let participants = anu.participants
                for (let num of participants) {
                    var ppuser;
                    try {
                        ppuser = await KingBot.profilePictureUrl(num, 'image')
                    } catch {
                    ...
                    if (Config.antifake != '') {
                        ...
                        for (let i = 0; i < array.length; i++) {
                            ...
                            if (chab === array[i]) {
                                console.log(array[i])
                                try {
                                    KingBot.sendMessage(anu.id, { text: `${chab} is not allowed` })
                                    return await KingBot.groupParticipantsUpdate(anu.id, [num], 'remove')
                                } catch {
                                    console.log("error")
                                }
                            }
                    ...
                    if (checkinfo) {
                        ...
                        if (anu.action == 'add' && events == "true") {
                                    ...
                                    return await KingBot.sendMessage(anu.id, buttonMessage)
                                    } else{

                                return KingBot.sendMessage(anu.id,{text: welcome_messages.trim(),mentions:[num] })
                                    }
                        ...
                        if(/@pp/g.test(goodbye_messages)) {
                           let buttonMessage = {
                            ...
                            return KingBot.sendMessage(anu.id, buttonMessage)
                        } else {
                            return KingBot.sendMessage(anu.id, {text: goodbye_messages.trim(),mentions:[num] })
                            ...
                            var ppUrl;
                            try {
                                ppUrl = await KingBot.profilePictureUrl(num, 'image')
                            } catch {
                            ...
                            KingBot.sendMessage(anu.id, buttonMessage)
        ...
        //========================================================================================================================================
        KingBot.ev.on('contacts.upsert', (contacts) => {
            ...
            //========================================================================================================================================
        KingBot.ev.on('contacts.update', async update => {
                for (let contact of update) {
                    let id = KingBot.decodeJid(contact.id)
            ...
            //========================================================================================================================================
        KingBot.getName = (jid, withoutContact = false) => {

                id = KingBot.decodeJid(jid)

                withoutContact = KingBot.withoutContact || withoutContact
                ...
                if (id.endsWith("@g.us")) return new Promise(async(resolve) => {
                    ...
                    if (!(v.name.notify || v.subject)) v = KingBot.groupMetadata(id) || {}
                    ...
                    } : id === KingBot.decodeJid(KingBot.user.id) ?
            ...
            //========================================================================================================================================
        KingBot.sendContact = async(jid, kon, quoted = '', opts = {}) => {
                let list = []
                for (let i of kon) {
                    list.push({
                        displayName: await KingBot.getName(i + '@s.whatsapp.net'),
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await KingBot.getName(i + '@s.whatsapp.net')}\nFN:${global.OwnerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:GitHub\nitem3.URL:https://github.com/${global.github}/Secktor-Md\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                    })
                }
                KingBot.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
            }
            //========================================================================================================================================
        KingBot.setStatus = (status) => {
            KingBot.query({
                tag: 'iq',
                attrs: {
                    to: '@s.whatsapp.net',
        ...
        KingBot.serializeM = (citel) => smsg(KingBot, citel, store)
            //========================================================================================================================================
        ...
        KingBot.ev.on('creds.update', saveCreds)
             ...
             */
            //========================================================================================================================================
        KingBot.send5ButImg = async(jid, text = '', footer = '', img, but = [], thumb, options = {}) => {
            let message = await prepareWAMessageMedia({ image: img, jpegThumbnail: thumb }, { upload: KingBot.waUploadToServer })
            ...
            KingBot.relayMessage(jid, template.message, { messageId: template.key.id })
        }
         ...
         */
        //========================================================================================================================================
        KingBot.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
            let buttonMessage = {
                    text,
                    footer,
                    buttons,
                ...
                //========================================================================================================================================
            KingBot.sendMessage(jid, buttonMessage, { quoted, ...options })
        }
         ...
         */
        //========================================================================================================================================
        KingBot.sendText = (jid, text, quoted = '', options) => KingBot.sendMessage(jid, { text: text, ...options }, { quoted })
         ...
         */
        //========================================================================================================================================
        KingBot.sendImage = async(jid, path, caption = '', quoted = '', options) => {
            let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split `,` [1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
            return await KingBot.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })
        }
         ...
         */
        //========================================================================================================================================
        KingBot.sendTextWithMentions = async(jid, text, quoted, options = {}) => KingBot.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
         ...
         */
        //========================================================================================================================================
    KingBot.sendImageAsSticker = async (jid, buff, options = {}) => {
      let buffer;
      ...
      await KingBot.sendMessage(
         ...
         * @returns
         */
KingBot.sendVideoAsSticker = async (jid, buff, options = {}) => {
      let buffer;
      if (options && (options.packname || options.author)) {
      ...
      await KingBot.sendMessage(
        ...
        options
            ...
            //========================================================================================================================================
        KingBot.sendMedia = async(jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
                let types = await KingBot.getFile(path, true)
             ...
             */
            //========================================================================================================================================
        KingBot.downloadAndSaveMediaMessage = async(message, filename, attachExtension = true) => {
            ...
            //========================================================================================================================================
        KingBot.downloadMediaMessage = async(message) => {
            let mime = (message.msg || message).mimetype || ''
            let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
         ...
         */
        //========================================================================================================================================
        KingBot.copyNForward = async(jid, message, forceForward = false, options = {}) => {
            let vtype
            ...
            await KingBot.relayMessage(jid, waMessage.message, { messageId: waMessage.key.id })
            return waMessage
        }
        KingBot.sendFileUrl = async(jid, url, caption, quoted, options = {}) => {
            let mime = '';
            let res = await axios.head(url)
            mime = res.headers['content-type']
            if (mime.split("/")[1] === "gif") {
                return KingBot.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options }, { quoted: quoted, ...options })
            }
            let type = mime.split("/")[0] + "Message"
            if (mime === "application/pdf") {
                return KingBot.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options }, { quoted: quoted, ...options })
            }
            if (mime.split("/")[0] === "image") {
                return KingBot.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options }, { quoted: quoted, ...options })
            }
            if (mime.split("/")[0] === "video") {
                return KingBot.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options }, { quoted: quoted, ...options })
            }
            if (mime.split("/")[0] === "audio") {
                return KingBot.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options }, { quoted: quoted, ...options })
            }
        }

        //========================================================================================================================================
        KingBot.cMod = (jid, copy, text = '', sender = KingBot.user.id, options = {}) => {
            ...
            copy.key.fromMe = sender === KingBot.user.id
         ...
         */
        //========================================================================================================================================
        KingBot.getFile = async(PATH, save) => {
                let res
            ...
            //========================================================================================================================================
        KingBot.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
                let types = await KingBot.getFile(PATH, true)
            ...
            //========================================================================================================================================
        KingBot.parseMention = async(text) => {
